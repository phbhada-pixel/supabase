    ...
    try {
        ...
        formsToProcess.forEach(formObj => {
            ...
            currentMonthResp.forEach(res => {
                ...
                // 🟢 Role Filter Logic: Check if employee role matches selected role
                if (filterRole !== "सर्व") {
                    if (!empObj || String(empObj.role).trim() !== filterRole) return;
                }
                ...
                validDataFields.forEach(f => {
                    if (f.formula && String(f.formula).trim() !== "") {
                        ...
                        if (isProgressive && f.type === 'number') {
                            ...
                            row.row.values[f.id] = formatNumberDecimals(evalRowFormula('M'));
                        }
                    }
                });
        ...
        if(finalReports.length > 0) { 
            ...
            alert(`⚠️ तुम्ही निवडलेल्या कालावधीचा डेटा फिल्टरमुळे लपवला जात आहे.\n\nकृपया खात्री करा की तुम्ही निवडलेले 'उपकेंद्र', 'वर्ष' आणि 'पद (Role)' अचूक आहेत.`); 
        }
